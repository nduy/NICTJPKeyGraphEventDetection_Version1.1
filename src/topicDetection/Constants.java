...
public class Constants {
	...
	public Constants(DataInputStream constantsFile) throws Exception {
        ...
        public Constants(File constantsFile) throws Exception {
		loadUtf8(constantsFile);
	}
	...
	public void load(DataInputStream in) throws Exception {
		...
		while ((line = in.readLine()) != null) {
			line = line.trim();
                        if (line.startsWith("// KEYGRAPH CONST")) startPoint= true;
			if (line.startsWith("//") || !startPoint || line.length() == 0)
				continue;
        ...
        public void loadUtf8(File in) throws Exception {
		// System.out.println(new File(constantsFile).getAbsolutePath());
		// DataInputStream in=new DataInputStream(new
		// FileInputStream(constantsFile));
		HashMap<String, String> conf = new HashMap<String, String>();
                 BufferedReader bufferedReader = new   BufferedReader(new InputStreamReader(new FileInputStream( in), "UTF-8"));
