...
import sharedModule.ValueComparator;
import static topicDetection.Main.g;

public class GraphAnalyze {
	Constants constants;

	public HashMap<String, Node> graphNodes;
        public static boolean enablePreferedKeywordsTFIDFCheck = true;
        
	public PrintStream logger = System.out;
        public static int MAX_KEYWORDS = 400;
        //public static List<String> preferredWords = new LinkedList<>(Arrays.asList("大雨","豪雨","土砂降り","どしゃ降り"));
        public static List<String> preferredWords = new LinkedList<>();
        
	public GraphAnalyze(Constants cons) {
		constants = cons;
	}

	public void buildGraph(HashMap<String, Document> documents, HashMap<String, Double> DF, boolean removeDuplicates) throws IOException {
		logger.println("build Graph");
		graphNodes = new HashMap<String, Node>();
                
                
                // --- Buid inverted index for TF: Term Frequency, frequency matrix
                HashMap<String,HashMap<String,Integer>> invertedIndex = new HashMap<>(); // save the frecency of each words in that document in a HashMap [wordID, count]. Then, the set of all these hash list represent the wholw corpus
                HashMap<String,Integer> docWithTerm = new HashMap(); // Count doc with term
                for (Document d: documents.values()){
                    HashMap<String,Integer> records =  new HashMap<>();
                    for (Keyword keyw: d.keywords.values()){
                        String word = keyw.baseForm;
                        if (records.containsKey(word)) continue;
                        int count = 0;
                        for (Keyword tkey: d.keywords.values()) if (tkey.baseForm.equals(word)) count++;
                        if (count>0) {
                            records.putIfAbsent(word, count);
                            if (docWithTerm.containsKey(word)){
                                docWithTerm.put(word, docWithTerm.get(word) + 1);
                            } else {            //add new
                                docWithTerm.put(word,1);
                            }
                        }
                    }
                    invertedIndex.put(d.id, records);
                }
                
                if (enablePreferedKeywordsTFIDFCheck){
                    //---Compute TF-IDF and remove low score words
                System.out.println("Computing TF-IDF");
                List<String> toRemoveDocs = new LinkedList<>();
                for (Document d: documents.values()){
                    //System.out.println("->> Doc " + d.keywords.values().toString());
                    if (d.keywords.size()<=MAX_KEYWORDS) continue; // Skipt docs that have less than or equal MAX_KEYWORDS words
                    Map<String,Float> records = new HashMap<>(); // To save the TF-IDF
                    ValueComparator bvc =  new ValueComparator(records);
                    TreeMap<String,Float> sorted_map = new TreeMap<String,Float>(bvc); // Sort the map DESCCENSEDING
                    Map<String,Integer> occurances = invertedIndex.get(d.id);
                    for (Keyword keyw: d.keywords.values()){
                        float tf = occurances.get(keyw.baseForm).floatValue()/ occurances.size();
                        float idf = (float) Math.log((float)documents.size() / docWithTerm.get(keyw.baseForm));
                        float tfidf = tf * idf;
                        records.put(keyw.baseForm, tfidf);
//                        if (keyw.baseForm.equals("大雨")||keyw.baseForm.equals("土砂降り")||keyw.baseForm.equals("豪雨")||keyw.baseForm.equals("どしゃ降り"))
//                        System.out.println("->>>> TFIFD of " + keyw.baseForm + " in doc " + d.id + " is " + tfidf);
                    }
                   
                    sorted_map.putAll(records);
                     List<String> toRemoveList = new LinkedList<>();
                     int count= 0;
                     for (Entry<String,Float> entry : sorted_map.entrySet()){
                         if (count++ > MAX_KEYWORDS-1)  toRemoveList.add(entry.getKey());
                    }
                   //  System.out.println("->>>> To remove items ");
                  //   for (String str : toRemoveList) System.out.println("            " + str);
                     // Then remove low tf-idf words
                     for (String str: toRemoveList ) documents.get(d.id).keywords.remove(str);
                     

                    if (!TFIDFCheck(records, preferredWords)) toRemoveDocs.add(d.id);
                }
                
                // Remove doc that have low tf-idf of prefered words 
                System.out.println( toRemoveDocs.size() + " out of " + documents.size() + " is deleted");
                for (String did: toRemoveDocs) documents.remove(did);
		...
		for (Document d : documents.values())
			if (!removeDuplicates || !d.isDuplicate)
				for (Keyword k : d.keywords.values()) {
					...
					if (graphNodes.containsKey(k.baseForm)){
						...
						graphNodes.put(keyword.baseForm, n);
                                                // add the node
                                                node = g.addNode(n.keyword.baseForm);
                                                node.addAttribute("ui.label", n.keyword.baseForm);
                ...
                logger.println("-> " + graphNodes.size()+ "  nodes added");
		...
		for (String baseForm : toRemoveIds){
                    ...
                    // Remove from graph
                    g.removeNode(baseForm);
                }
			
		toRemoveIds.clear();
                logger.println("-> nodes filtered");
		...
		for (Document d : documents.values())
			if (!removeDuplicates || !d.isDuplicate){
                //            System.out.println("--> Process doc: " + d.id + ". Keywords size: " + d.keywords.size() );
                            for (Keyword k1 : d.keywords.values()){
                                 //System.out.println("----> Process keyword: " + k1.baseForm);
							...
							if (graphNodes.containsKey(k2.baseForm) && k1.baseForm.compareTo(k2.baseForm) < 0) {
								...
								if (!n1.edges.containsKey(edgeId)) {
									...
									n2.edges.put(edgeId, e);
                                                                        // Add edge
                                                                        g.addEdge(edgeId, n1.keyword.baseForm, n2.keyword.baseForm);
								}
								n1.edges.get(edgeId).df++;
                                                                
                                                                ///////////////////////////////////// Check spartial and temporal info here
                                                                if (Main.inClustersIDs.contains(d.id)) n1.edges.get(edgeId).df_incluster++;
		...
		for (Node n : graphNodes.values()) {
			for (Edge e : n.edges.values()) {
				...
				e.n2.edges.remove(e.id);
                                // Remove edge
                                g.removeEdge(e.n1.keyword.baseForm , e.n2.keyword.baseForm);
			}
                        toRemove.clear();
		}
                // -- Remove out-of-cluster edgles
                if (Main.enable_ST_cluster_restrict){
                    for (Node n : graphNodes.values()) {
			for (Edge e : n.edges.values()) {
                            e.computeSTscore();
                            if (e.stscore < Main.MIN_STSCORE) {
                                System.out.println(e.df_incluster +" " + e.df  +" " + e.stscore);
                                toRemove.add(e);
                            }
		...
		toRemoveIds.clear();
                logger.println("Finished build Graph");
	}
	...
	public ArrayList<HashMap<String, Node>> extractCommunities(HashMap<String, Node> nodes) {
		...
		while (connectedComponents.size() != 0) {
			...
			// communities.add(subNodes);
                ...
                // Remove all node that is not in community
                List<String> toKeep = new LinkedList();
                List<String> toRemove = new LinkedList();
                for (HashMap<String, Node> has: communities){
                    for (Node n: has.values()){
                        toKeep.add(n.keyword.baseForm);
                    }
                }
                for (org.graphstream.graph.Node n : g.getNodeSet()){
                    if (!toKeep.contains(n.getId())) toRemove.add(n.getId());
                }
                
                for (String str: toRemove) g.removeNode(str);
	...
	public static HashMap<String, Node> mergeKeyGraphs(HashMap<String, Node> kg1, HashMap<String, Node> kg2) {
        ...
        /***
         * This method will check if the preferred words have TF-IDF greater than the average TF-IDF of all words in the documents 
         * @param records : a Hashmap specified for a document, whose value are: <words,TF-IDF of word in document>
         * @param preferedWords: list of preferred words
         * @return true if 1. maximum TF-IDF of preferred words greater the average, 2. the document does not contain any preferred words
         */
        public boolean TFIDFCheck(Map<String,Float> records, List<String> preferedWords){
            float average = 0;
           
            boolean containsPreferredWords = false;
            for (String item: preferedWords){
                if (records.containsKey(item)) {
                    containsPreferredWords =true;
                    break;
                }
            }
            if (containsPreferredWords){
                for (String word: records.keySet())
                average = average + records.get(word);
                average = average/records.size();

                // Check if TF-IDF of prefred words is> average
                // - Find max TF-IDF of prefered keywords
                float max = 0;
                for (String key : preferedWords ){
                    float score = records.containsKey(key) ? records.get(key) : 0;
                    max = Math.max(max, score);
                }

                if (max < average) return false;
            }
