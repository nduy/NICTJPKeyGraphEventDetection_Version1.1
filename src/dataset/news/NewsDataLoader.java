...
public class NewsDataLoader extends DataLoader {
	...
	public void loadDocumentKeyFilesAll(HashMap<String, Document> docs, HashSet<String> stopwords, HashMap<String, Double> DF) {
		...
		try {
			...
			String[] files = f.list();
			loadDocumentsForSpinn3r(files, docs, stopwords, DF);
	...
	public void loadDocumentKeyFilesByTopics(ArrayList<Topic> events, HashMap<String, Document> docs, HashSet<String> stopwords,
		...
		try {
			...
			for (int i = 0; i < files.length && events.size() < constants.TOPIC_MAX; i++) {
				...
				// continue;
				Topic e = loadDocumentKeyFilesByTopic(docs, stopwords, DF, avg, df,openDataInputStream(constants.DATA_TOPIC_PATH+files[i]),
						files[i]);
	...
	public void loadDocumentKeyFilesByTopics(ArrayList<Topic> events, HashMap<String, Document> docs, HashSet<String> stopwords,
		...
		try {
			...
			for (int i = 0; i < files.length; i++) {
				Topic e = loadDocumentKeyFilesByTopic(docs, stopwords, DF, avg, df, files[i], beginDate, endDate);
				if (e.docs.size() >= constants.TOPIC_MIN_SIZE) {
					events.add(e);
					avg += e.docs.size();
	...
	public Topic loadDocumentKeyFilesByTopic(HashMap<String, Document> docs, HashSet<String> stopwords, HashMap<String, Double> DF, double avg,
			DateFormat df, InputStreamReader reader, String eventId) throws Exception {
		...
		while ((line = bufferedReader.readLine()) != null && line.trim().length() > 0) {
			...
			constants.KEYWORDS_2_ENABLE) {
				...
				if (exists(ff))
					loadDocumentKeyFile(openDataInputStream(ff), stopwords, d, 1);
				else
					System.out.println(ff + " cannot be found!");
			}
			if (constants.TEXT_ENABLE) {
				...
				if (exists(ff))
					loadDocumentTextFile(openDataInputStream(ff), stopwords,  d, constants.TEXT_WEIGHT,false,null);
				else
					System.out.println(ff + " cannot be found!");
			}
	...
	private Topic loadDocumentKeyFilesByTopic(HashMap<String, Document> docs, HashSet<String> stopwords, HashMap<String, Double> DF, double avg,
			DateFormat df, String file, Timestamp beginDate, Timestamp endDate) throws Exception {
		Topic e = new Topic(file);

		InputStreamReader reader = openDataInputStream(constants.DATA_TOPIC_PATH+"/"+file);
                 BufferedReader bufferedReader = new   BufferedReader(reader);
		String line = null;
		int j = 0;
		while ((line = bufferedReader.readLine()) != null && line.trim().length() > 0) {
